...
export function WordsPage() {
  ...
  return <>
    ...
    <section className="section compact-top"><div className="container">
      <div className="words-toolbar"><label><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={text('搜索字词、粤拼或意思', '搜尋字詞、粤拼或意思')} /></label><p><b>{filtered.length}</b> / 100 {text('个常用字词', '個常用字詞')}</p></div>
      <div className="word-list">{filtered.map(([simp, trad, jp, meaning, example], index) => { const saved = isWordSaved(simp); return <article className="word-row" key={simp}><span className="word-index">{String(index + 1).padStart(2, '0')}</span><div className="word-main"><h2>{text(simp, trad)}</h2><p>{jp}</p><AudioButton compact /></div><div className="word-meaning"><span>{text('白话意思', '白話意思')}</span><p>{text(meaning)}</p></div><div className="word-example"><span>{text('这样说', '這樣說')}</span><strong>{text(example)}</strong><AudioButton compact /></div><button className={saved ? 'word-save saved' : 'word-save'} onClick={() => { if (!isLoggedIn) navigate('/login?next=/words'); else void toggleWord({ simplified: simp, traditional: trad, jyutping: jp, meaning, example }) }}><Bookmark fill={saved ? 'currentColor' : 'none'} /></button></article> })}</div>
    </div></section>
  </>
}
