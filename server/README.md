...
第一段的 350 条字幕来自自动转写，仍属于待校对测试内容；第二段的 40 条粤语正文已经项目负责人校对，粤拼和普通话释义仍标注为 AI 辅助初稿。
...
当前已实现 `POST /api/v1/ai/explain-subtitle`，由后端调用 DeepSeek 生成结构化字幕讲解。未配置密钥或调用失败时不会影响视频、字幕和收藏功能。
